    ...
    public function summary(): array
    {
        ...
        return [
            'kpis' => [
                ...
                'receivables' => $this->metric($receivableAccounts !== [] || $invoices !== [], $receivableAccounts !== [] ? collect($receivableAccounts)->sum('balance') : collect($currentInvoices)->sum('remaining_balance'), count($currentInvoices).' open '.Str::plural('invoice', count($currentInvoices))),
                'payables' => $this->metric($payableAccounts !== [], collect($payableAccounts)->sum('balance'), count($payableAccounts).' '.Str::plural('account', count($payableAccounts))),
                ...
                'overdue_payables' => $this->metric(false, 0, 'No bill data available'),
            ],
            'monthly' => $monthly,
