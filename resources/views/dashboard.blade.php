            ...
            <table class="w-full min-w-190 text-left text-xs">
                ...
                <tbody class="text-slate-700">
                    @forelse ($dashboard['recent_journals'] as $entry)
                        ...
                        <tr class="apm-table-row"><td><a href="{{ route('journal-entries', ['entry' => $entry['journal_number']]) }}" class="font-mono text-[11px] font-medium text-blue-600 hover:underline">{{ $entry['journal_number'] }}</a></td><td class="font-medium text-slate-800">{{ $entry['description'] }}</td><td class="font-mono text-[11px]">{{ $entry['date'] }}</td><td>{{ $entry['source_type'] }}</td><td class="apm-money text-right">₱{{ number_format($entry['total_debit'], 2) }}</td><td><span class="inline-flex rounded-md px-2 py-1 text-[10px] font-medium {{ $statusClass }}">{{ $entry['status'] }}</span></td></tr>
                    @empty
